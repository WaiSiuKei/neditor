...
import { runWhenIdle, IdleDeadline } from '@neditor/core/base/common/async';
...
export const WorkbenchContrib = 'workbench.contributions.kind';
...
class WorkbenchContributionsRegistry implements IWorkbenchContributionsRegistry {
  ...
  private readonly toBeInstantiated = new Map<LifecyclePhase,
  ...
  private instantiateByPhase(
    ...
    // Otherwise wait for phase to be reached
      ...
      lifecycleService
        .when(phase)
        .then(() => this.doInstantiateByPhase(instantiationService, phase));
    }
  }
...
Registry.add(WorkbenchContrib, new WorkbenchContributionsRegistry());
