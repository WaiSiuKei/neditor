...
export class SimpleKeybinding {
  ...
  public modifierCount(): number {
    return Number(this.ctrlKey) + Number(this.shiftKey) + Number(this.altKey) + Number(this.metaKey);
  }

  public codeCount(): number {
    return this.isDuplicateModifierCase() ? this.modifierCount() : this.modifierCount() + this.keyCode === KeyCode.None ? 0 : 1
  }
...
export function createSimpleKeybindingFromKeyCodes(keyCodes: Set<KeyCode>, OS: OperatingSystem): SimpleKeybinding {
  let ctrlKey = false;
  let shiftKey = false;
  let altKey = false;
  let metaKey = false;

  let keyCode = KeyCode.None
  for (let key of keyCodes) {
    if (key === KeyCode.Ctrl) ctrlKey = true
    else if (key === KeyCode.Shift) shiftKey = true
    else if (key === KeyCode.Alt) altKey = true
    else if (key === KeyCode.Meta) metaKey = true
    else {
      keyCode = key
    }
  }
...
export function createKeybinding(keybinding: number, OS: OperatingSystem): Keybinding | null {
  ...
  if (chordPart !== 0) {
    throw new Error('chordkeybinding is not su');
  }
