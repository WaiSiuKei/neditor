...
const patches: Patch[] = [];
export function trackMutation(target: object, type: TriggerOpTypes, key: string | symbol, newValue: unknown, oldValue?: unknown) {
...
export function trackParentChain(target: object, key: string, parent: object) {
  if (editingMode !== EditingMode.update) return;
  parentMap.set(target, { key, parent });
}

function getPath(target: object): [object, string[]] {
...
export function redoPatches(target: object, patches: Patch[]): void {
...
export function undoPathes(target: object, patches: Patch[]): void {
