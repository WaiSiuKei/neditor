...
import { isSafari } from '@neditor/core/base/browser/browser';
import { IndexedDB } from '@neditor/core/base/browser/indexedDB';
import { Promises } from '@neditor/core/base/common/async';
import { toErrorMessage } from '@neditor/core/base/common/errorMessage';
import { Emitter } from '@neditor/core/base/common/event';
import { Disposable, IDisposable, toDisposable } from '@neditor/core/base/common/lifecycle';
import {
...
import {
  ...
  StorageScope,
  StorageTarget,
} from '@neditor/core/platform/storage/common/storage';
...
export class BrowserStorageService extends AbstractStorageService {
  ...
  protected async doInitialize(): Promise<void> {
    ...
    const [workspaceStorageDatabase, globalStorageDatabase] = await Promises.settled([
      ...
      IndexedDBStorageDatabase.create(
        {
          id: this.getId(StorageScope.GLOBAL),
          broadcastChanges: true /* only for global storage */,
        },
        this.logService,
      ),
    ]);
    ...
    // Init both
    await Promises.settled([this.workspaceStorage.init(), this.globalStorage.init()]);
...
export class IndexedDBStorageDatabase extends Disposable implements IIndexedDBStorageDatabase {
  ...
  private constructor(
    ...
    this.broadcastChannel =
      options.broadcastChanges && 'BroadcastChannel' in window
        ? new BroadcastChannel(IndexedDBStorageDatabase.STORAGE_BROADCAST_CHANNEL)
        : undefined;
