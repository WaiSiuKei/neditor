...
import { Point } from '../../../../base/common/geometry/point';
import { BrushVisitor } from '../brushVisitor';
...
function IsNearColorStopList(lhs: ColorStopList,
                             rhs: ColorStopList,
                             epsilon: number) {
...
//    ccw_radians_from_right: number,  frame_size: SizeF): [ Point,  Point]
// {
// }
...
class LinearGradientBrushData {
  source_: Point = new Point();
  dest_: Point = new Point();
  ...
  EQ(other: LinearGradientBrushData): boolean {
    return this.source_.EQ(other.source_) && this.dest_.EQ(other.dest_) &&
      equals(this.color_stops_, other.color_stops_, (a: ColorStop,
                                                     b: ColorStop) => a.EQ(b));
  }
...
export class LinearGradientBrush extends Brush {
  ...
  constructor(source: Point,
              dest: Point,
              source_color: ColorRGBA,
              dest_color: ColorRGBA)
  constructor(source: Point,
              dest: Point,
              a3: unknown,
              a4?: unknown) {
  ...
  source(): Point { return this.data_.source_; }
  dest(): Point { return this.data_.dest_; }
...
export class RadialGradientBrush extends Brush {
  private center_: Point = new Point();
