...
import { trackParentChain, trackMutation, isMutating, deepClone } from './patch';
...
function set(target: object,
  ...
  if (isArray(target) && key === 'length') {
    ...
    if (num < oldValue) {
      for (let i = oldValue - 1; i >= num; i--) {
        trackMutation(receiver, TriggerOpTypes.DELETE, i, undefined, deepClone(target[i]));
      }
    }
  }
...
function deleteProperty(target: object,
