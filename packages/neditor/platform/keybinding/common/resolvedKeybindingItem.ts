import { ResolvedKeybinding } from '@neditor/core/base/common/keyCodes';
import { CharCode } from '@neditor/core/base/common/charCode';
import { ContextKeyExpression } from '../../contextkey/common/contextkey';

export class ResolvedKeybindingItem {
  declare _resolvedKeybindingItemBrand: void;
  ...
  constructor(
    ...
    this.keypressParts = resolvedKeybinding
      ? removeElementsAfterNulls(resolvedKeybinding.getDispatchParts())
      : [];
    if (resolvedKeybinding && this.keypressParts.length === 0) {
      // handle possible single modifier chord keybindings
      this.keypressParts = removeElementsAfterNulls(
        resolvedKeybinding.getSingleModifierDispatchParts(),
      );
    }
