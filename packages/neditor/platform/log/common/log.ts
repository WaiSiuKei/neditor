...
import { isWindows } from '@neditor/core/base/common/platform';
...
export abstract class AbstractLoggerService extends Disposable implements ILoggerService {
  ...
  constructor(private logLevel: LogLevel, onDidChangeLogLevel: Event<LogLevel>) {
    super();
    this._register(
  ...
  createLogger(resource: URI, options?: ILoggerOptions): ILogger {
    ...
    if (!logger) {
      logger = this.doCreateLogger(
        resource,
        options?.always ? LogLevel.Trace : this.logLevel,
        options,
      );
